...
fun main(args: Array<String>) {
    ...
    // If CH write succeeds but ES fails, Flink rolls back Kafka offset.    env.enableCheckpointing(config.checkpointingInterval)

    // FIX: Configure Kryo serialization for better Kotlin data class support
    // Register custom serializers for Kotlin data classes    env.config.registerKryoType(UnifiedEvent::class.java)
    ...
    // Strategy: Heavy Batching. ClickHouse thrives on large inserts.    // PITFALL FIX: The "Real-Time Write Trap".    // We force Flink to buffer up to 5000 rows or 5 seconds, whichever comes first.//    val jdbcExecutionOptions = JdbcExecutionOptions.builder()
//        .withBatchSize(5000)
